...
pub fn main() {
    ...
    // since [pk_s * pk_r]G1 = c_2 - m.0
    // we can retrieve message index by checking whether two pairings are equivalent
    // e([pk_s * pk_r]G1, hash_c) = e([pk_r]G1, [pk_s]hash_c)

    // 1. we must get [hash_c]G2 for further pairing
    let hash_c_g2 = blob.c.hash_to_curve();

    // 2. iterate all messages to fine the corresponding one
    messages.iter().enumerate().for_each(|(idx, msg)| {
        // 3. get [pk_s * pk_r]G1
        let pks_pkr_g1 = blob.c.1 - msg.0;

        let e_pks_pkr_g1_hash_c = Bls12_381::pairing(pks_pkr_g1, hash_c_g2);

        // 4. get e([pk_r]G1, [pk_s]hash_c)
        let e_rec_g1_auth_g2 = Bls12_381::pairing(blob.rec_pk, blob.s);

        if e_pks_pkr_g1_hash_c == e_rec_g1_auth_g2 {
            println!("message idx = {idx}");
        }
    });
