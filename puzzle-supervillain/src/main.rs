...
fn main() {
    ...
    let old_proof_new_idx = public_keys
        .iter()
        .enumerate()
        .fold(G2Projective::zero(), |acc, (i, (_, proof))| {
            let new_idx = Fr::from(new_key_index as u64 + 1).div(Fr::from(i as u64 + 1));
            acc + proof.mul(new_idx)
        })
        .into_affine();

    let old_aggregate_key_neg = G1Affine::zero() - old_aggregate_key;

    let new_sk = {
        let rng = &mut ark_std::rand::rngs::StdRng::seed_from_u64(39915u64);
        Fr::rand(rng)
    };

    let new_key = (old_aggregate_key_neg + G1Affine::generator().mul(new_sk)).into_affine();
    let new_proof = (pok_prove(new_sk, new_key_index) - old_proof_new_idx).into_affine();
